...
const loginDetailsUser = router.get("/login_details_user/:id",async (req,res,next)=>{
  ...
  const currentdate= String(addZero(date.getFullYear())) + "-" + String(addZero(cMonth)) + "-" + String(addZero(date.getDate()));

  const result = await request.query(`select * from login_details WHERE e_id='${id}' AND date='${currentdate}' `);
